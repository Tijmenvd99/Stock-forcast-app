...
if ticker:
    ...
    if len(df) < horizon + 1:
        st.warning(f"Niet genoeg recente data om {horizon_label} vooruit te voorspellen.")
    else:
        ...
        time_ahead = timedelta(hours=horizon) if unit == "hours" else timedelta(days=horizon)
        prediction_date = df.index[-1] + time_ahead
        ...
        if prediction_date > datetime.now():
            ...
            st.warning("⚠️ De voorspelling ligt in het verleden en wordt daarom niet weergegeven op de grafiek.")
